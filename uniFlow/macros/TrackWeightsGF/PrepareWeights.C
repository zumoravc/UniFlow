...
TH3D* ProcessWeights3D(const TH3D* weight = 0x0);
...
void PrepareWeights()
{
  ...
  if(!bRunByRun)
  {
    ...
    for(Short_t part = 0; part < iNumPart; part++)
    {
      ...
      TH2D* h2Weights = ProcessWeights(h3Weights);
      // CheckWeights(h3Weights, iRunList[iRun]);
      h2Weights->SetName(Form("%s_pvz%d",species[part].Data()));
...
//_____________________________________________________________________________
TH2D* ProcessWeights3D(const TH3D* weights)
{
  if(!weights) { printf("PrepareWeights: no input histo\n"); return 0x0;}

  TH3D* weights_3d = (TH3D*) weights->Clone();
  ...
  for(Short_t binX(0); binX < iNumBinX+2; binX++)
    for(Short_t binY(0); binY < iNumBinY+2; binY++)
      for(Short_t binZ(0); binZ < iNumBinZ+2; binZ++)
      {
        dContent = weights_2d->GetBinContent(binX,binY,binZ);
        if(dContent > 0.0) { weights_3d->SetBinContent(binX,binY,binZ, dMax / dContent); }
        else {weights_3d->SetBinContent(binX,binY,binZ, 1.0); }
        weights_3d->SetBinError(binX,binY,binZ,0.0);
      }
