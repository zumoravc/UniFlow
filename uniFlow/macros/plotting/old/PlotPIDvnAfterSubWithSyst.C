...
void PlotPIDvnAfterSubWithSyst()
{
  SetStyle();
  ...
  TString sOutputPath = Form("%s/results-test/",sInputPathTop.Data());
  gSystem->mkdir(sOutputPath.Data(),kTRUE);

  TFile* fileOut = TFile::Open(Form("%s/v2-syst-final.root",sInputPathTop.Data()),"RECREATE");
  ...
  for(Int_t iCent(0); iCent < iNumCent; ++iCent)
  {
    ...
    TLegend* leg = new TLegend(0.18,0.5,0.4,0.79);
    ...
    for(Int_t spec(0); spec < iNumSpecies; ++spec)
    {
      ...
      Double_t dPt[30] = {0.0};
      Double_t dPtErr[30] = {dPtBinErr};
      Double_t dPtSyst[30] = {dPtBinSyst};
      ...
      // TCanvas* test = new TCanvas("test","test",800,500);
      // test->Divide(2,1);
      // test->cd(1);
      // histPoints->Draw();
      // test->cd(2);
      // graphPoints->Draw();
      // graphSyst->Draw("e2 same");
      // graphPoints->Draw("p e same");
    }
    ...
    canPlot->cd();
    TH1* frame_canPlot = (TH1*) gPad->DrawFrame(0.0,-0.02,7.0,0.30);
    frame_canPlot->SetTitle(Form("; p_{T} (GeV/c); v_{2}^{sub} {2, |#Delta#eta| > %s}",sGapVal.Data()));
    text2->DrawLatexNDC(0.186,0.85,Form("ALICE Preliminary"));
    text->DrawLatexNDC(0.186,0.81,Form("p-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV (%s%% V0A)", sCent[iCent].Data()));
    leg->Draw();
