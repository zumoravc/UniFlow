...
Color_t colSystAfterBarlow = kGreen+2;

Int_t markBase = kFullSquare;
Int_t markSyst = kFullCircle;
Int_t markSystAfterBarlow = kFullSquare;
...
)
{
    ...
    // fit Diff
    TF1* fitDiff = nullptr;
    // if(syst.EqualTo("PID3sigma")) {
    //     fitDiff = new TF1("fitDiff","[0]",2.0,xmax);
    // } else {
    ...
    StyleHist(histAfterBarlow,colSystAfterBarlow,markSystAfterBarlow);
    TF1* fitAfterBarlow = new TF1("fitAfterBarlow","[0]",xmin,xmax);
    fitAfterBarlow->SetLineColor(kGreen);
    ...
    can->cd(3);
    TH1* frame2 = (TH1*) gPad->DrawFrame(xmin,1.0-dRatioToler,xmax,1.0+dRatioToler);
    histRatio->DrawCopy("same e1");
    lineUnity->DrawLine(xmin,1.0,xmax,1.0);
...
TH1D* ApplyBarlow(TH1* diff, TH1* barlow, Double_t dCut)
{
  ...
  for(Int_t iBin(1); iBin < after->GetNbinsX()+1; ++iBin) {
    ...
    after->SetBinContent(iBin, dContent);
