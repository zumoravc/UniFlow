...
void PlotMethods()
{
  ...
  for(Int_t iBase = 0; iBase < iNumBases; ++iBase)
  {
    ...
    for(Int_t iCent = 0; iCent < iNumCent; ++iCent)
    {
      ...
      TString sBaseLineLabels[] = {Form("pp (%s)",sCentLabel.Data()), "pp (MB, 0-100%)","pPb (60-100%)"};

      TCanvas* canMethod = new TCanvas("canMethod","canMethod", 400,400);
      canMethod->cd(1);
      TH1* frame_canMethod = (TH1*) gPad->DrawFrame(dPtmin,dYmin,dPtmax,dYmax);
      frame_canMethod->SetTitle(Form("%s (%s); p_{T} (GeV/c); v^{sub}_{2}{2,|#Delta#eta| < %.1f}",sSpecies.Data(), sCentLabel.Data(),dGapValue));

      TLegend* legMethods = new TLegend(0.12,0.55,0.4,0.85);
      legMethods->SetFillColorAlpha(0,0);
      legMethods->SetBorderSize(0);
      legMethods->SetHeader(Form("pPb - %s",sBaseLineLabels[iBase].Data()));
      ...
      for(Int_t iMethod(0); iMethod < iNumMethods; ++iMethod)
      {
        ...
        StyleHist(hist, colorsMethods[iMethod],markersMethods[iMethod]);
        legMethods->AddEntry(hist,sMethodLabels[iMethod].Data(),"p");
      ...
      TString sOutput = Form("%s/comp-methods/%s/",sPathTop.Data(),sGap.Data());
      gSystem->mkdir(sOutput.Data(),kTRUE);
      canMethod->SaveAs(Form("%s/vn_subt_%s_cent%d.pdf",sOutput.Data(),sBase.Data(),iCent),"pdf");
    }
  }
