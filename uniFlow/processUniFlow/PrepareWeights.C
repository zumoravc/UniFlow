...
const TString sPath = "/Users/vpacik/NBI/Flow/results/uniFlow_syst/baseline_noNUA/FAST_16t/";
...
void PrepareWeights()
{
  ...
  TFile* fOutput = new TFile(Form("%s/weights_Cor_FAST_16t.root",sPath.Data()),"RECREATE");
  ...
  for(Short_t iRun = 0; iRun < iNumRuns; iRun++)
  {
    ...
    for(Short_t part = 0; part < iNumPart; part++)
    {
      ...
      // h2Weights = CheckWeights(h3Weights, iRunList[iRun]);
