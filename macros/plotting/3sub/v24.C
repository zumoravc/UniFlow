...
void v24(Int_t part, Int_t centClass){
  ...
  TFile* fileIn = TFile::Open("/home/alidock/output/LHC15o/train_7456/processUniFlow/Processed_V0.root ","READ");
  ...
  TGraphErrors* gap08 = nullptr;
  if(false) {
    gap08 = new TGraphErrors((TH1D*) fileV->Get(Form("%s_hFlow4_harm2_gap00_cent%d",specLong.Data(),centClass)));
  ...
  TGraphErrors* vp = nullptr;
  if(centClass > 0 && centClass < 7){
    vp = new TGraphErrors((TH1D*) fileV->Get(Form("%s_hFlow4_harm2_gap00_cent%d",specLong.Data(),centClass-1)));
    vp->SetMarkerStyle(kOpenSquare);
  ...
  can->SaveAs(Form("ptdif/comp_VP/v24_pPb_%s_cent%d_VP.pdf",spec.Data(),centClass));
 ...
 //  TH1D* hisV = (TH1D*) fileV->Get(Form("%s_hFlow4_harm2_gap00_cent%d",specLong.Data(),centClass-1));
 //
 //  TLegend* leg2 = new TLegend(0.12,0.75,0.55,0.88);
 //  TF1 *fu1 = new TF1("fu1", "pol0", 0.4, 10.);
 //
 ...
 //
 //  hisV->Draw("h");
 //  hisV->GetYaxis()->SetRangeUser(0.9,1.1);
 ...
 //  fu1->Draw("same");
 //
 //  if(false) cRat->SaveAs(Form("ptdif/comp_VP/v24_PbPb_%s_cent%d_VP_ratio.pdf",spec.Data(),centClass));
 //
 ...
 //
 //      legGr->AddEntry(graph[poiPos][twoPos],Form("v_{2}{4} (poi: %c, 2p: %c)",sides[poiPos],sides[twoPos]),"p");
 //
 //
 //      gStyle->SetOptStat(kFALSE);
 //      gStyle->SetOptFit(1);
 ...
 // legGr->Draw("same");
 //
 // cGr->SaveAs(Form("ptdif/v24_pb_%s_cent%d_combi.pdf",spec.Data(),centClass));
