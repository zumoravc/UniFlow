...
void Subt_ppb_pp_species_gap(TString sSpecies, TString sGap, TString sTag, TString sType, TString sOutPath);
...
// TString sType = "tracking"; TString sTags[] = {"FB","PV","cls"}; TString sSpecies_list[] = {"Charged","Pion","Kaon","Proton","K0s","Lambda","Phi"};
// TString sType = "pid"; TString sTags[] = {"3sigma","bayes90"}; TString sSpecies_list[] = {"Pion","Kaon","Proton","Phi"};
// TString sType = "v0s"; TString sTags[] = {"3sigma","decayRad","DCAdaughters","CPA"}; TString sSpecies_list[] = {"K0s","Lambda"};
// TString sType = "fitting"; TString sTags[] = {"K0s/flowBG","K0s/massBG","K0s/massSig","K0s/range1","K0s/range2","K0s/range3","K0s/range4","K0s/range5","K0s/range6","K0s/range7"}; TString sSpecies_list[] = {"K0s"};
// TString sType = "fitting"; TString sTags[] = {"Lambda/flowBG","Lambda/massBG","Lambda/massSig","Lambda/range1","Lambda/range2","Lambda/range3","Lambda/range4","Lambda/range5"}; TString sSpecies_list[] = {"Lambda"};
TString sType = "fitting"; TString sTags[] = {"Phi/massSig"}; TString sSpecies_list[] = {"Phi"};
...
void Subt_ppb_pp_syst()
{
  ...
  for(Int_t g(0); g < iNumGaps; ++g)
  {
    for(Int_t t(0); t < iNumTags; ++t)
    {
      ...
      TString sOutPath = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-pPb-16qt-nua/subt/output_binning-3/"+sGapBase+"/"+sTag+"/";
      // TString sOutPath = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-16q-nua/subt/output_binning-3/"+sGapBase+"/"+sTag+"/";
      ...
      {
        Subt_ppb_pp_species_gap(sSpecies_list[i], sGapBase, sTag, sType, sOutPath);
      }
    }
...
void Subt_ppb_pp_species_gap(TString sSpecies, TString sGap, TString sTag, TString sType, TString sOutPath)
{
  ...
  TString sInFileRaw = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-pPb-16qt-nua/output_binning-3/" + sGapBase + "/"+sTag+"/"; // + "/" + sMethod;
  // TString sInFileRaw = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-16q-nua/output_binning-3/"+sGapBase+"/"+sTag+"/";
  TString sInFileBaseInt = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/merged-pp-16kl-nua/output_binning-3/" + sGapBase; // + "/"; + sMethod;
  // TString sInFileBaseInt = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/merged-pp-16kl-nu/output_binning-3-test/" + sGapBase; // + "/"; + sMethod;
  // TString sOutFolder = sInFileRaw+"/"+sMethod+"/pPb_pp_subt_"+sGapRaw+"/"+sSpecies;

  TString sOutFolder = sOutPath+"/"+sSpecies+"/"; // + "/" + sMethod;
  ...
  canMult->SaveAs(Form("%s/Mult.pdf",sOutFolder.Data()));
  ...
  canRef->cd(2);
  hSubt_cn->DrawCopy();
  canRef->SaveAs(Form("%s/Refs.pdf",sOutFolder.Data()));
  ...
  //dn
    ...
    for(Int_t cnBin(1); cnBin < hSubt_cn->GetNbinsX()+1; ++cnBin)
    {
      ...
      canDiff->cd(2);
      hSubt_dn->DrawCopy();
      canDiff->SaveAs(Form("%s/%s_cent%d.pdf",sOutFolder.Data(),sSpecies.Data(),cent));
