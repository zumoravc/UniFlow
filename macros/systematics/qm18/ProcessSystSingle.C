...
// TString sType = "fitting"; TString sTags[] = {"K0s/flowBG","K0s/massSig","K0s/massBG","K0s/range1","K0s/range2","K0s/range3","K0s/range4","K0s/range5","K0s/range6","K0s/range7"}; TString sSpeciesList[] = {"K0s"};
// TString sType = "fitting"; TString sTags[] = {"Lambda/flowBG","Lambda/massSig","Lambda/massBG","Lambda/range1","Lambda/range2","Lambda/range3","Lambda/range4","Lambda/range5"}; TString sSpeciesList[] = {"Lambda"};
TString sType = "fitting"; TString sTags[] = {"Phi/flowBG","Phi/massBG","Phi/massSig","Phi/range1","Phi/range2","Phi/range3","Phi/range4","Phi/range5","Phi/range6"}; TString sSpeciesList[] = {"Phi"};

// TString sOutputPath = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/test/"+sType+"/";
...
void ProcessSystSingle()
{
  gSystem->mkdir(sOutputPath.Data(),kTRUE);

  // ProcessSyst(iCent,iTag,iSpecies,bCorrelated);

  for(Int_t tag(0); tag < iNumTags; ++tag)
    for(Int_t species(0); species < iNumSpecies; ++species)
      for(Int_t cent(0); cent < iNumCent; ++cent)
      {
        ProcessSyst(cent,tag,species,bCorrelated);
        // ProcessSyst(cent,iTag,species,bCorrelated);
        // ProcessSyst(cent,iTag,iSpecies,bCorrelated);
      }
...
void ProcessSyst(Int_t iCent, Int_t iTag, Int_t iSpecies, Bool_t bCorrelated)
{
  ...
  // TString sInputBase = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/merged-pPb-16qt-nua/output_binning-3/"+sGap+"/Processed.root";
  ...
  // TString sInputBase = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/merged-pPb-16qt/subt/output_binning-3/"+sGap+"/Subtracted.root";
  TString sInputSyst = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-pPb-16qt-nua/subt/output_binning-3/"+sGap+"/"+sTag+"/Subtracted.root";
  // TString sInputSyst = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-16qt/subt/output_binning-3-new/"+sGap+"/"+sTag+"/Subtracted.root";
  // TString sInputSyst = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/"+sType+"/merged-16q-nua/output_binning-3/"+sGap+"/"+sTag+"/Processed.root";

  //  --------------------------------------------------------
  gSystem->mkdir(Form("%s%s",sOutputPath.Data(),sTag.Data()),kTRUE);
  fileOutput = TFile::Open(Form("%s/%s.root",sOutputPath.Data(),sTag.Data()),"UPDATE");
  ...
  TH1D* histRatio = DivideHistos(histSyst,histBase,bCorRatio);
  ...
  TF1* fitRatio = new TF1("fitRatio",sFitFunc.Data(),dFitXmin,dFitXmax);
  histRatio->Fit("fitRatio","MNIR");

  TF1* fitRatioDiff = new TF1("fitRatioDiff",Form("TMath::Abs((%s)-1)",sFitFunc.Data()),dFitXmin,dFitXmax);
  fitRatioDiff->SetLineStyle(2);
  for(Int_t iPar(0); iPar < iNumParFit; ++iPar){ fitRatioDiff->SetParameter(iPar, fitRatio->GetParameter(iPar)); }
  ...
  listOut->Add(fitRatioDiff);
  // listOut->Add(fitDiff);

  TString sListName = Form("%s_cent%d",sSpecies.Data(),iCent);
  printf("%s",sListName.Data());

  fileOutput->cd();
  // fileOutput->ls();
  gDirectory->Delete(Form("%s_cent%d;*",sSpecies.Data(),iCent));
  // fileOutput->ls();
  listOut->Write(sListName.Data(),TObject::kSingleKey);
  ...
  for(Int_t iPar(0); iPar < iNumParFit; ++iPar)
  {
    latex->DrawLatex(0.2,0.70-iPar*0.05,Form("#color[2]{a%d = %.3f #pm %.3f}",iPar,fitRatio->GetParameter(iPar), fitRatio->GetParError(iPar)));
  }
  ...
  gPad->SetRightMargin(0.03);
  gPad->SetLeftMargin(0.15);
  TH1* frame_diff = (TH1*) gPad->DrawFrame(histDiff->GetXaxis()->GetXmin(),dDiffYmin,histDiff->GetXaxis()->GetXmax(),dDiffYmax);
  frame_diff->SetTitle(Form("|1 - (x_{syst} / x_{default})|; p_{T} (GeV/c); Relative diff."));
  frame_diff->SetLabelFont(43,"XY");
  frame_diff->SetLabelSize(14,"XY");
  frame_diff->SetTitleFont(43,"XY");
  frame_diff->SetTitleSize(14,"XY");
  frame_diff->SetTitleOffset(2.1,"Y");
  frame_diff->SetTitleOffset(1.5,"X");
  histDiff->DrawCopy("same");
  fitRatioDiff->Draw("same");
