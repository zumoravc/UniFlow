...
void RunSystematics_FB()
{
	...
	TString sTopOutput = "/Users/vpacik/NBI/Flow/uniFlow/results/qm-run/syst/tracking/merged-16qt";
	TString sTags[] = {"FB"};
	Int_t iNumTags = 1;
	...
	for(Int_t iGap(0); iGap < iNumGaps; ++iGap)
	{
		...
		for(Int_t iTag(0); iTag < iNumTags; ++iTag)
		{
			...
			TString sOutputFilePath = sTopOutput+"/output_binning-3/"+sEtaGap+"/"+sTag+"/";
