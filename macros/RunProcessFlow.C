...
void RunProcessFlow()
{
	...
	//Double_t etaGaps[] = {1.};
	Double_t etaGaps[] = {-1.,1.}; // only NoGap case
	Short_t sizeEtaGaps = sizeof(etaGaps) / sizeof(etaGaps[0]);

	//Double_t cent[] = {0.,5.,10.,20,30,40,50,60,70,80}; //edges
	Double_t cent[] = {0,30,50,150}; //edges
	Short_t sizeCent = sizeof(cent) / sizeof(cent[0]) - 1; 

	ProcessFlow* process = new ProcessFlow();
	process->SetDebug(0);
	process->SetInputFilePath("~/NBI/Flow/results/pp-2016k-QA2-2/merge/");
	process->SetOutputFilePath("~/NBI/Flow/results/pp-2016k-QA2-2/");
	process->SetOutputFileName("Flow_FB768_kINT7.root");
	process->SetTag("FB768_kINT7");
