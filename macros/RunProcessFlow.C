...
void RunProcessFlow()
{
	...
	Double_t cent[] = {0,50,70,150}; // edges
	Short_t sizeCent = sizeof(cent) / sizeof(cent[0]) - 1;

	ProcessFlow* process = new ProcessFlow();
	process->SetDebug(1);
	process->SetInputFilePath("~/NBI/Flow/results/v2-PID-pp-lhc16k");
	process->SetOutputFilePath("~/NBI/Flow/results/v2-PID-pp-lhc16k/plots");
	process->SetOutputFileName("Flow_kINT7_FB96.root");
	process->SetTag("kINT7_FB96");
