...
AliAnalysisTaskFlowPID* AddTaskFlowPID(TString name = "name")
{
  ...
  fileName += ":FlowPID";      // create a subfolder in the file
  ...
  mgr->ConnectOutput(task,1,mgr->CreateContainer("Tracks", TList::Class(), AliAnalysisManager::kOutputContainer, fileName.Data())); // same for the output
  mgr->ConnectOutput(task,2,mgr->CreateContainer("V0s", TList::Class(), AliAnalysisManager::kOutputContainer, fileName.Data())); // same for the output
  mgr->ConnectOutput(task,3,mgr->CreateContainer("QA", TList::Class(), AliAnalysisManager::kOutputContainer, fileName.Data())); // same for the output
  return task;
}
